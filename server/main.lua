...
AddEventHandler('esx_garage:updateOwnedVehicle', function(stored, parking, Impound, data, spawn)
    ...
    MySQL.update(
        'UPDATE owned_vehicles SET `stored` = @stored, `parking` = @parking, `impound` = @Impound, `vehicle` = @vehicle WHERE `plate` = @plate AND `owner` = @identifier',
        {
    ...
    if stored then
        ...
        ESX.OneSync.SpawnVehicle(data.vehicleProps.model, spawn, data.spawnPoint.heading, data.vehicleProps,
            function(vehicle)
...
AddEventHandler('esx_garage:setImpound', function(Impound, vehicleProps)
    ...
    MySQL.update(
        'UPDATE owned_vehicles SET `stored` = @stored, `impound` = @Impound, `vehicle` = @vehicle WHERE `plate` = @plate AND `owner` = @identifier',
        {
            ['@identifier'] = xPlayer.identifier,
...
ESX.RegisterServerCallback('esx_garage:getVehiclesInPound', function(source, cb, Impound)
    ...
    MySQL.query('SELECT * FROM `owned_vehicles` WHERE `owner` = @identifier AND `impound` = @Impound AND `stored` = 2',
        {
            ['@identifier'] = xPlayer.identifier,
            ['@Impound']    = Impound
        }, function(result)
